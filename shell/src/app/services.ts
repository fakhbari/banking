declare global {
  interface Window {
    services:{pluginName:string , serviceList:{[index: string]:any}}[]
  }
}

export const callServiceOfPlugin = (pluginName:string,serviceName:string) =>{
  const foundPlugin = window.services.filter((service:{pluginName:string , serviceList:{[index: string]:any}}) => service.pluginName === pluginName)
